...
bM > 0.55 → Con este primer modelo, al ser positivo con un intervalo de credibilidad del 95% entre 0.49 y 0.60
            creemos que los hombres de la muestra tienden a juzgar más permisivo intervenir.

Paso 2: Añadir control por intención y acción
```{r}
m2g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
El modelo tiene menor WAIC por lo que decimos que el ajuste mejora unu poco

Paso 3: Añadir edad (estandarizada)
```{r}
m3g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
Evalúa si la edad tiene un efecto (bAge).
Compara con modelos anteriores con compare(m2, m3).

Paso 4: Añadir interacciones entre male y intent, action
```{r}
m4g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
bMI: Si positivo → hombres son más permisivos cuando la acción es intencional.
bMA: Si negativo → hombres menos permisivos cuando la acción es directa (acción == 1).
## Resultados
