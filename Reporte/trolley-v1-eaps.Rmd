...
### Modelo sencillo incluyendo sólo género
```{r, message=FALSE, warning=FALSE}
m1g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
### Añadir control por intención y acción
```{r, message=FALSE, warning=FALSE}
m2g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
```{r}
# Extraer y seleccionar coeficientes
posterior_samples_m3g <- extract.samples(m3g)
draws_df <- data.frame(
  Acción = posterior_samples_m3g$bA,
  Intención = posterior_samples_m3g$bI,
  Género = posterior_samples_m3g$bM,
  Edad = posterior_samples_m3g$bAge
)
...
### Añadir interacciones entre género, intención y acción
```{r, message=FALSE, warning=FALSE}
m4g <- ulam(
  alist(
    response ~ dordlogit(phi, cutpoints),
...
```{r}
# Extraer y seleccionar coeficientes
posterior_samples_m4g <- extract.samples(m4g)
draws_df <- data.frame(
  Acción = posterior_samples_m4g$bA,
  Intención = posterior_samples_m4g$bI,
  Género = posterior_samples_m4g$bM,
  Edad = posterior_samples_m4g$bAge,
  Género-intención = posterior_samples_m4g$bMI,
  Género-acción = posterior_samples_m4g$bMA
)
...
## Análisis del Efecto de la Edad sobre las Decisiones Morales

### Modelos Considerados

Se evaluó el impacto de la edad en los juicios morales usando dos enfoques complementarios:

- **Edad como variable continua**: Se estandarizó la edad (`age_s`) y se incluyó en el modelo ordinal como predictor lineal. Esto permite identificar posibles **tendencias globales** en la permisividad moral conforme aumenta la edad. El modelo asume una relación lineal entre la edad y la propensión a aceptar moralmente la acción evaluada:
...
- **Edad por grupos (G1–G6)**: Se agruparon las edades en intervalos de 10 años, combinando `[60,70)` y `[70,80)` en un solo grupo final para mejorar la estabilidad del modelo. Cada grupo de edad tiene su propio efecto estimado jerárquicamente con regularización bayesiana:
...
Este enfoque por grupos permite capturar **patrones no lineales** en la relación entre edad y juicio moral, ofreciendo una interpretación más flexible que complementa la perspectiva continua.

### Resultados Principales

#### Distribución de Respuestas (edad por grupos)

```{r, echo=FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics("figs/edad_group_dist_respuestas_grid.png")
```

- Los grupos intermedios (G3–G5) tienden a mostrar mayor permisividad moral (respuestas 6 y 7).

- Los extremos etarios (G1 y G6) presentan mayor variabilidad, reflejando menos consistencia en sus respuestas.

- La intención del agente es el factor más determinante en todos los grupos.

#### Probabilidad de Respuesta 7 (edad por grupos)

```{r, echo=FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics("figs/edad_group_prob_cat7_grid.png")
```

- El grupo G1 (más joven) muestra mayor probabilidad de considerar moralmente aceptables los escenarios con baja intencionalidad.

- Los grupos intermedios tienden a mantener una posición más crítica y estable.

- En escenarios con `intention = 1`, todos los grupos reducen la permisividad, aunque G6 se mantiene más indulgente.

#### Distribución de Respuestas (edad como variable continua)

```{r, echo=FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics("figs/edad_cont_dist_respuestas_grid.png")
```

- El análisis con edad continua muestra patrones suaves y crecientes de permisividad, aunque con menor diferenciación entre extremos.

- No se detectan cambios abruptos, lo que sugiere que los grupos pueden capturar mejor patrones no lineales.

### Consideraciones Causales

Aunque los modelos ajustados permiten identificar asociaciones claras entre la edad y los juicios morales, **no es posible interpretar estos efectos como causales**. Esto se debe a que la muestra utilizada proviene de una encuesta voluntaria en línea, lo cual introduce un **sesgo de selección no controlado**. En términos causales, la **participación en el estudio** actúa como un **colisionador** entre variables como la edad, el nivel educativo y otros factores no observados que influyen en la disposición a participar. Al condicionar implícitamente sobre esta participación, se rompe la independencia entre causas potenciales y se imposibilita estimar efectos causales totales.

En particular, el efecto de la edad sobre los juicios morales podría estar mediado o confundido por el nivel educativo, y **no es identificable** en esta estructura de datos. Por tanto, los resultados deben interpretarse como **asociaciones condicionales** dentro de la muestra observada, y no como inferencias generalizables al conjunto de la población. Este análisis ilustra la importancia de considerar explícitamente las estructuras causales subyacentes —y sus limitaciones— al trabajar con datos observacionales.


## Análisis del Efecto del Nivel Educativo sobre las Decisiones Morales

### Exploración y Modelado del Nivel Educativo

Para analizar cómo la educación influye en los juicios morales, agrupamos los niveles educativos en seis categorías ordenadas (G1–G6), desde educación básica hasta posgrado. Se ajustó un modelo logit ordinal donde el término lineal incluye los efectos principales (`action`, `contact`, `intention`), sus interacciones, y un efecto específico por grupo educativo estimado de forma jerárquica:
...
\phi = \beta_A \cdot \text{action} +
       ...
       \beta_{AI} \cdot (\text{action} \cdot \text{intention}) +
       \beta_{CI} \cdot (\text{contact} \cdot \text{intention}) +
       \beta_{E[g]}, \quad \beta_{E[g]} \sim \text{Normal}(0, \sigma_{\text{edu}})
\]

Este enfoque permite capturar variabilidad entre niveles sin asumir una relación funcional específica, incorporando regularización bayesiana para evitar sobreajuste.

### Resultados Principales

Los gráficos muestran cómo varía la distribución estimada de respuestas:

```{r, echo=FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics("figs/educacion_cont_dist_respuestas_grid.png")
```

y la probabilidad de emitir el juicio más permisivo (respuesta = 7) según el nivel educativo y el tipo de dilema moral:

```{r, echo=FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics("figs/educacion_group_prob_cat7_grid.png")
```

- **Patrones consistentes en niveles medios-altos:** Los grupos educativos G4–G6 tienden a presentar mayor probabilidad de respuestas permisivas (categoría 7), especialmente cuando `intention = 1`, sugiriendo que los participantes con mayor escolaridad otorgan más peso a la intención al juzgar.

- **Variabilidad en extremos:** Los niveles más bajos (G1, G2) muestran mayor incertidumbre y menor consistencia en sus respuestas, lo que puede reflejar tanto heterogeneidad de criterios como menor tamaño muestral.

- **El contexto importa:** El efecto del nivel educativo varía según la configuración del dilema. La intención tiende a aumentar sistemáticamente la permisividad, pero este efecto se amplifica en los niveles más altos de escolaridad.

### Consideraciones Causales

Aunque el análisis revela asociaciones claras entre nivel educativo y juicio moral, **no podemos atribuirle un efecto causal directo a la educación**. Esto se debe nuevamente a que la participación en esta encuesta es voluntaria y está influenciada por factores como edad, género o escolaridad, todos ellos relacionados entre sí y con la decisión de participar. En términos causales, la **participación actúa como colisionador** en el DAG implícito del estudio.

Por tanto, intentar estimar efectos causales directos de variables estructurales como la educación requeriría modelar o evitar la selección por participación, lo cual no es posible con estos datos. En consecuencia, los resultados deben interpretarse como **asociaciones condicionales** entre educación y juicio moral, útiles para generar hipótesis, pero no para evaluar intervenciones educativas.
